...
namespace UnityEditor.TerrainTools
{
    internal class BrushRenderWithTerrainUIGroup : IBrushRenderWithTerrain
    {
        ...
        public PaintContext AcquireHeightmap(bool writable, Terrain terrain, Rect boundsInTerrainSpace, int extraBorderPixels = 0)
        {
            ...
            // BeginPaintHeightmap会创建sourceRenderTexture和destinationRenderTexture
            // 然后将当前地型高度图与笔刷高度图相交区域保存到sourceRenderTexture中
        ...
        public PaintContextExp AcquireCurrentLayerHeightmap(bool writable, Terrain terrain, Rect boundsInTerrainSpace, int extraBorderPixels = 0)
        {
            m_WriteToHeightmap = writable;
            m_HeightmapContext = TerrainExpandTools.BeginPaintHeightmap(terrain, boundsInTerrainSpace, extraBorderPixels);
