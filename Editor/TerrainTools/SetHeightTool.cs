...
namespace UnityEditor.TerrainTools
{
    ...
    internal class SetHeightTool : TerrainPaintTool<SetHeightTool>
    {
        ...
        {
            if (!TerrainEditorConfig.CanEditor(terrain))
            {
                return;
            }
            ...
            if (!TerrainExpandConfig.Instance.isCanEditorTerrain)
            {
                return;
            }
        ...
        private Material ApplyBrushFromBaseHeightInternal(PaintContextExp paintContext, IBrushRenderUnderCursor brushRender, float brushStrength, Texture brushTexture, UnityEngine.TerrainTools.BrushTransform brushXform, Terrain terrain)
        {
            Material mat = GetPaintMaterial();//TerrainExpandTools.GetPaintHeightExtMat();
            ...
            Graphics.Blit(paintContext.sourceRenderTexture, paintContext.destinationRenderTexture, mat, 0/*(int)TerrainBuiltinPaintMaterialPasses.SetHeights*/);
                ...
                else
                {
                    ...
                    using (IBrushRenderUnderCursor brushRender = new BrushRenderUIGroupUnderCursor(commonUI, "SetHeightTool", brushTexture))
                    {
                        if (brushRender.CalculateBrushTransform(out BrushTransform brushTransform))
                        {
                            ...
                            //刷到自定义的当前层次高度图中
                            //将当前编辑层次的地型高度图与笔刷相交的区域blit到笔刷中，用于后续的高度计算
                            PaintContextExp paintContextTmp = TerrainExpandTools.BeginPaintHeightmap(terrain, brushBounds);
                            ApplyBrushFromBaseHeightInternal(paintContextTmp, brushRender, commonUI.brushStrength, editContext.brushTexture, brushTransform, terrain);

                            for (int i = 0; i < paintContextTmp.terrainCount; ++i)
                            {
                                if (paintContextTmp.GetTerrain(i).gameObject.TryGetComponent<TerrainExpand>(out TerrainExpand terrainExpandData))
                                {
                                    terrainExpandData.OnPaint(paintContextTmp, i, (Event.current.shift ? -1 : 1));
                                    TerrainExpandConfigGUI.Instance.SetDirty(terrain);
                                }
                            }
