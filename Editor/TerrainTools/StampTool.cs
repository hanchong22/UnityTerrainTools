...
namespace UnityEditor.TerrainTools
{
    ...
    internal class StampTool : TerrainPaintTool<StampTool>
    {
        ...
        {
            if (!TerrainExpandConfig.Instance.isCanEditorTerrain || !TerrainEditorConfig.CanEditor(terrain))
            {
                return false;
            }
            ...
            if (commonUI.allowPaint && Event.current == null || Event.current.type != EventType.MouseDrag)
            {
                ...
                using (IBrushRenderUnderCursor brushRender = new BrushRenderUIGroupUnderCursor(commonUI, "Stamp", brushTexture))
                {
                    ...
                    {
                        Rect brushBounds = brushXform.GetBrushXYBounds();
                        var paintContext = brushRender.AcquireCurrentLayerHeightmap(true, brushBounds);
                        ...
                        {
                            if (paintContext.GetTerrain(i).gameObject.TryGetComponent<TerrainExpand>(out TerrainExpand terrainExpandData))
                            {
                                terrainExpandData.OnPaint(paintContext, i, (Event.current.shift ? -1 : 1));
                                TerrainExpandConfigGUI.Instance.SetDirty(terrain);
                            }
                        }

                        //将其它layer合并到画笔中，然后调用内置画笔功能
                        List<int> leftLayers = ListPool<int>.Get();
                        for (int i = -2; i < TerrainExpandConfig.Instance.HeightMapCount; ++i)
                        {
                            if (i != TerrainExpandConfig.Instance.CurrentHeightLayer)
                            {
                                if (i < 0 || TerrainExpandConfig.Instance.HeightMaps[i].isValid)
                                {
                                    leftLayers.Add(i);
                                }
                            }
                        ...
                        paintContext.GatherLeftHeightmap(leftLayers);
                        ListPool<int>.Release(leftLayers);
                    }
                }
            }
            return true;
            ...
            if (evt.type == EventType.Repaint && commonUI.isRaycastHitUnderCursorValid)
            {
                ...
                using (IBrushRenderPreviewUnderCursor brushRender = new BrushRenderPreviewUIGroupUnderCursor(commonUI, "Stamp", brushTexture))
                {
                    if (brushRender.CalculateBrushTransform(out BrushTransform brushXform))
                    {
                        ...
                        var paintContext = brushRender.AcquireCurrentLayerHeightmap(false, brushXform.GetBrushXYBounds(), 1);
