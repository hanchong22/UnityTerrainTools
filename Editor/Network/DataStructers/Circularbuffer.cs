...
public class CircularBuffer : Stream
{
    ...
    public override long Length
    {
        get
        {
            ...
            if (c < 0)
            {
                UnityEngine.Debug.Log(string.Format("CircularBuffer count < 0: {0}, {1}, {2}", this.bufferQueue.Count, this.LastIndex, this.FirstIndex));
            }
            return c;
        }
    }
    ...
    public void ReadFrom(Stream stream, uint iLen)
    {
        int count = (int)(stream.Length - stream.Position);
        count = (int)Math.Min(iLen, count);
